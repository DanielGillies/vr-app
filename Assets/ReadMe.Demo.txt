This is a demo version of  MPMP (multi platform media player). (version  1.7)
It has the same capabilities as the official version from the Assetstore but draws a watermark on top of the videos and the texture in linear colorspace on Windows is not sRGB.
